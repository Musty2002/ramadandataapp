...
export function ServicesGrid() {
  ...
  return (
    <div className="mx-4 my-6 p-4 sm:p-5 bg-gradient-to-br from-primary to-primary/80 rounded-2xl shadow-lg">
      ...
      <div className="grid grid-cols-4 gap-3 sm:gap-4">
        {services.map(({ icon: Icon, label, path, comingSoon }) => (
          ...
          >
            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center transition-all ${
              comingSoon ? '' : 'group-hover:scale-110 group-hover:bg-white/30 group-active:scale-95'
            }`}>
              <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
            </div>
            <span className="text-[10px] sm:text-xs text-white/90 font-medium text-center leading-tight">{label}</span>
            {comingSoon && (
              <span className="absolute -top-1 -right-1 text-[7px] sm:text-[8px] bg-white text-primary px-1 sm:px-1.5 py-0.5 rounded-full font-semibold">
                Soon
              </span>
            )}
          </button>
